<template>
  <view class="case">
    ...
    <scroll-view v-if="showNavScroll" scroll-x="true" class="bg-white nav fixed" scroll-with-animation="true" :scroll-left="scrollLeft" :style="'padding-top:' + (CustomBar + 15) + 'px;'">
      <block v-for="(item, index) in caseNav" :key="index">
        <view :class="['cu-item text-lg', index == TabCur ? ' text-black cur text-bold text-xl' : '']" @click='navSelect(item.id, index)'>
          {{item.title}}
        </view>
      </block>
    </scroll-view>
    ...
    <view class="cu-card article no-card padding-top-sm" :style="'padding-top:' + CustomBar + 'px;padding-bottom:' + CustomBar + 'px;'"
      ...
      <view v-else class="" :style="'padding-top:' + CustomBar + 'px;'">
          <block class="" v-for="(item, index) in categoryData" :key="index">
            <view class="cu-card">
              <view @click="navDetail(item.id)" class="cu-item bg-img shadow-blur" :style="'background-image:url(' + item.main_image.prefix + ')'">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      navDetail(id) {
        uni.navigateTo({
          url: '/pagesA/case-detail/case-detail?id=' + id,
        });
      },
